    ...
    private func resolveWordUnderCursorPath(at point: NSPoint? = nil) -> WordPathResolution? {
        ...
        if ghostty_surface_quicklook_word(surface, &text) {
            ...
            if text.offset_len > 0,
               let offsetStart = Int(exactly: text.offset_start),
               offsetStart >= 0 {
#if DEBUG
                let viewportOffsetStart = cmuxTerminalCmdClickViewportOffsetDelta(offsetStart)
#else
                let viewportOffsetStart = offsetStart
#endif
