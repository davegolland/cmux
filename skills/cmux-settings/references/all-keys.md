...
| `terminal.renderMath` | boolean | `true` | Typeset LaTeX math printed by terminal programs ($...$, \(...\), $$...$$, \[...\]) as an overlay over the raw text in every terminal. Set false for plain text. Toggle live with the toggleTerminalMathRendering shortcut or the command palette. |
