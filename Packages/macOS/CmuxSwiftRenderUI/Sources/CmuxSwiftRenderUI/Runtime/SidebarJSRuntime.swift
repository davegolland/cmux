...
/// Bounds deferred host actions while JavaScript event callbacks are active.
/// The runtime is main-actor isolated, but the closures execute later on the
/// main queue and retain their validated payload until then. A lock keeps the
/// admission counter safe if a future JavaScriptCore callback arrives off the
/// owning thread.
private final class SidebarDeferredActionAdmission: @unchecked Sendable {
    private let lock = NSLock()
...
public final class SidebarJSRuntime {
    ...
    @ObservationIgnored private let deferredActionAdmission = SidebarDeferredActionAdmission(
        maximumPending: SidebarSecurityLimits.maxPendingActions
    )
    ...
    /// Invalidates deferred actions from a previous source revision. A queued
    /// closure must not run after the sidebar has been reloaded or replaced.
    ...
    private func runAction(json: String) {
        ...
        guard deferredActionAdmission.reserve() else {
            sidebarJSRuntimeLogger.warning("Dropped sidebar action because the deferred action queue is full")
            return
        }
        let dispatch = self.dispatch
        let admission = deferredActionAdmission
        let generation = actionGeneration
        DispatchQueue.main.async { [weak self, admission] in
