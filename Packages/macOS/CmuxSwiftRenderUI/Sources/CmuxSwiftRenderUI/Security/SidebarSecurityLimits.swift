...
public enum SidebarSecurityLimits {
    ...
    /// Maximum UTF-8 path size accepted by sidebar file and directory
    /// boundaries. This keeps path normalization and filesystem APIs from
    /// receiving an attacker-sized string.
    public static let maxFilePathBytes = 4 * 1024
    ...
    /// Maximum sidebar actions waiting for the host main-actor turn.
    ///
    /// Actions are deferred so a click can paint before a host command runs.
    /// Admission control keeps an authored event flood from retaining an
    /// unbounded number of closures and command payloads on the main queue.
    public static let maxPendingActions = 256
...
enum SidebarBoundedFileReader {
    ...
    static func data(from url: URL, fileManager: FileManager = .default) throws -> Data {
        guard url.isFileURL,
              url.path.hasPrefix("/"),
              url.path.utf8.count <= SidebarSecurityLimits.maxFilePathBytes else {
