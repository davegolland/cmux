...
public struct ArtifactByteReader: Sendable {
    ...
    private func sourceDimensionsAreBounded(_ source: CGImageSource) -> Bool {
        ...
        switch (sourcePixelDimension(properties[kCGImagePropertyPixelWidth]),
                sourcePixelDimension(properties[kCGImagePropertyPixelHeight])) {
        case (.absent, .absent):
            return true
        case let (.valid(width), .valid(height)):
            return width <= Int64(Self.maximumThumbnailSourceDimension)
                && height <= Int64(Self.maximumThumbnailSourceDimension)
                && width <= Int64(Self.maximumThumbnailSourcePixels) / height
        default:
            // A decoder that reports only one dimension, a non-numeric value,
            // or an overflowing NSNumber must fail closed. Treating an
            // unrepresentable value as "missing" would bypass the
            // decompression-bomb guard.
            return false
        }
    }

    private enum SourceDimension {
        case absent
        case valid(Int64)
        case invalid
    }

    private func sourcePixelDimension(_ value: Any?) -> SourceDimension {
        ...
        let integerValue = Int64(doubleValue)
        guard integerValue > 0, Double(integerValue) == doubleValue else {
            return .invalid
        }
        return .valid(integerValue)
    }
    ...
    public func kind(path: String, isDirectory: Bool) -> ChatArtifactKind {
        guard Self.isSafePath(path) else { return .binary }
        return kind(
            path: path,
            isDirectory: isDirectory,
            isRegularFile: nil
    ...
    private func validatePath(_ path: String) throws {
        guard Self.isSafePath(path) else { throw Error.readFailed }
    }

    private static func isSafePath(_ path: String) -> Bool {
        guard !path.isEmpty,
              path.utf8.count <= ChatArtifactSecurityLimits.maxPathBytes,
              path.utf8.first == 0x2F else {
            return false
        }
        return !path.unicodeScalars.contains {
